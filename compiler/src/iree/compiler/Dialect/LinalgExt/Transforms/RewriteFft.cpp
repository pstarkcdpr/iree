...
static SmallVector<Value> getBitReversalOrderComplex(ImplicitLocOpBuilder &b,
                                                     ...
                                                     bool hasScale,
                                                     double scale) {
  auto shapeType = llvm::cast<ShapedType>(complexTensor.getType());
  ...
  Value scaleValue;
  if (hasScale) {
    scaleValue = b.create<arith::ConstantOp>(
        complexElemType, b.getFloatAttr(complexElemType, scale));
  }
  ...
  auto genericOp = b.create<linalg::GenericOp>(
      ...
      [&](OpBuilder &b, Location loc, ValueRange args) {
        ...
        ivs.push_back(
            b.create<arith::IndexCastOp>(loc, b.getIndexType(), args[0]));
        auto elem = b.create<tensor::ExtractOp>(loc, complexTensor, ivs);
        auto realPart = b.create<complex::ReOp>(loc, complexElemType, elem);
        auto imagPart = b.create<complex::ImOp>(loc, complexElemType, elem);
        if (hasScale) {
          b.create<linalg::YieldOp>(loc, SmallVector<Value> {
...
static Value expandConjugateSymmetry(ImplicitLocOpBuilder &b,
                                     ...
                                     int64_t fftLength) {
  auto halfType = llvm::cast<RankedTensorType>(halfSizedComplex.getType());
  auto complexType = llvm::cast<ComplexType>(halfType.getElementType());
  ...
  Value emptyMirror = b.create<tensor::EmptyOp>(mirrorMixedSizes, complexType);
  ...
  auto mirrorOp = b.create<linalg::GenericOp>(
      TypeRange{mirrorType}, ValueRange{}, emptyMirror, maps, iterTypes,
      [&](OpBuilder &b, Location loc, ValueRange args) {
        // Mirror index: for output index i, we want input[N/2 - 1 - i]
        // which mirrors indices N/2-1, N/2-2, ..., 1
        Value idx = b.create<linalg::IndexOp>(loc, rank - 1);
        Value halfSizeMinusOne =
            b.create<arith::ConstantIndexOp>(loc, fftLength / 2 - 1);
        Value mirrorIdx = b.create<arith::SubIOp>(loc, halfSizeMinusOne, idx);
        ...
        Value elem = b.create<tensor::ExtractOp>(loc, halfSizedComplex, ivs);
        Value conjElem = b.create<complex::ConjOp>(loc, elem);

        b.create<linalg::YieldOp>(loc, conjElem);
      });

  // Concatenate first half and mirrored second half along the last dimension
  Value result = b.create<tensor::ConcatOp>(
      rank - 1,
...
FailureOr<std::pair<Value, Value>> rewriteFft(Operation *op,
  ...
  auto complexType = llvm::dyn_cast<ComplexType>(operandType.getElementType());
...
FailureOr<std::pair<Value, Value>> rewriteIrfft(Operation *op,
  ...
  b.create<Util::OptimizationBarrierOp>(results[1]);
  return std::make_pair(results[0], results[1]);
}
